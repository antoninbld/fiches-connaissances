...
assert.match(html, /ATLAS_INTERACTIVE_LAYERS=\[ATLAS_SELECTION_LAYER_ID,/);
assert.ok(click.indexOf('ATLAS_SELECTION_LAYER_ID') < click.indexOf('point_count!=null'), 'la sélection superposée doit être traitée avant le cluster situé dessous');
...
assert.match(html, /atlasMap\.on\('style\.load'.*ensureAtlasSourceAndLayers\(\{forceRecreate:true\}\).*raiseAtlasLayers\(\)/s);
